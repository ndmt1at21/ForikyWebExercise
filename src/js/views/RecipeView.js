...
class RecipeView extends View {
    ...
    _generateMarkupListIngredients() {
        let ingredients = ``;
        ...
        <li class="recipe__ingredient">
            ...
            <div class="recipe__quantity">${(_a = Math.round(ingr.quantity * 100) / 100) !== null && _a !== void 0 ? _a : ''}</div>
    ...
    <div class="recipe__details">
        ...
        <button class="btn--round btn--bookmark">
            <svg class="">
                <use href="${icons}#${this.data.bookmarked ? 'icon-bookmark-fill' : 'icon-bookmark'}"></use>
            </svg>
        </button>
    </div>
    ...
    <div class="recipe__ingredients">
        ...
        <ul class="recipe__ingredient-list">
            ${this._generateMarkupListIngredients()}
        </ul>
    </div>
    ...
    addHandlerRender(handler) {
        ['hashchange', 'load'].map(evtName => window.addEventListener(evtName, function (e) {
            handler();
        }));
    }
    addHandlerUpdateServings(handler) {
        this.parentElement.addEventListener('click', function (e) {
            ...
            if (!target)
                return;
            const servings = +target.dataset.updateTo;
            handler(servings);
        });
    }
